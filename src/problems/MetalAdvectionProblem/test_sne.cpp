...
AMREX_GPU_MANAGED Real hscale= 1000. * pc;
...
AMREX_GPU_MANAGED Real hscaleIa= 2. * 1000. * pc; //should be twice of typeII scale height
AMREX_GPU_MANAGED Real hscaleAGB= 2. * 1000. * pc; //should 300 pc.
		  ...
		  amrex::GpuArray<Real, AMREX_SPACEDIM> dx, SimulationData<NewProblem> const &userData, int level)
{
	...
	for (amrex::MFIter iter(mf); iter.isValid(); ++iter) {
		...
		// amrex::Print() << "The scale height of type II=%d"<< hscale << "\n";
		// amrex::Print() << "The scale height of type Ia=%d"<< hscaleIa << "\n";
		// amrex::Print() << "The scale height of type AGB=%d"<< hscaleAGB << "\n";
	}
}
